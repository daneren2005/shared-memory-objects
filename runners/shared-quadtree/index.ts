...
// around the world for several rounds, all concurrently, hammering the per-node locks. Afterwards a full-world retrieve
// must return exactly the surviving ids, each valid and unique: a torn link or lost update would show up as a garbage
// id, a duplicate, or a count mismatch.
...
const tree = new SharedQuadtree(heap, {
	bounds: WORLD,
...
window.setTimeout(() => {
	log(`running quadtree operations across ${WORKER_COUNT} workers`);
...
function checkIfDone() {
	...
	log(`finished quadtree operations in ${Math.round(performance.now() - startTime)}ms`);

	if(tree.size === totalExpected) {
		log(`all workers done - tree size ${tree.size} matches expected ${totalExpected}`);
	} else {
		warn(`all workers done - tree size ${tree.size} does NOT match expected ${totalExpected}`);
	}
	...
	let all = tree.retrieve(WORLD.x, WORLD.y, WORLD.width, WORLD.height);
	...
	if(unique.size !== tree.size) {
		warn(`unique retrieved ids ${unique.size} does NOT match tree size ${tree.size}`);
	}
