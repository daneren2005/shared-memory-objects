...
self.onmessage = (e) => {
	if(e.data.map) {
		...
		map = new SharedSpatialMap(heap, e.data.map);
		...
		for(let round = 0; round < updateRounds; round++) {
			for(let entity of live) {
				randomPosition(entity);
				map.update(entity.id, entity.x, entity.y, entity.width, entity.height);
			}
		...
		for(let entity of live) {
			...
			let found = map.retrieve(entity.x - 1, entity.y - 1, entity.width + 2, entity.height + 2);
