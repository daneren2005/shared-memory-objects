...
import SharedSpatialMap from '../../src/spatial/shared-spatial-map';
...
// around the world for several rounds, all concurrently, hammering the per-bucket locks. Afterwards a full-world retrieve
// must return exactly the surviving ids, each valid and unique: a torn link, lost update, or dropped multi-cell slot
// would show up as a garbage id, a duplicate, or a count mismatch. Unlike the grid this world has no fixed extent - the
// map hashes cells into a fixed bucket array, so distinct cells share buckets and the id+cell disambiguation is exercised.
...
const map = new SharedSpatialMap(heap, {
	gridSize: GRID_SIZE,
...
window.setTimeout(() => {
	log(`running spatial map operations across ${WORKER_COUNT} workers`);
...
function checkIfDone() {
	...
	log(`finished spatial map operations in ${Math.round(performance.now() - startTime)}ms`);

	if(map.size === totalExpected) {
		log(`all workers done - map size ${map.size} matches expected ${totalExpected}`);
	} else {
		warn(`all workers done - map size ${map.size} does NOT match expected ${totalExpected}`);
	}
	...
	let all = map.retrieve(WORLD.x, WORLD.y, WORLD.width, WORLD.height);
	...
	if(unique.size !== map.size) {
		warn(`unique retrieved ids ${unique.size} does NOT match map size ${map.size}`);
	}
